...
if (_PROJECT_ROOT / "ablation").is_dir() or (_PROJECT_ROOT / "baseline").is_dir():
    pass  # already at project root
else:
    _PROJECT_ROOT = _PROJECT_ROOT.parent  # script may sit in a subfolder
...
for _candidate in _CANDIDATE_MODULES:
    try:
        _model_mod = __import__(_candidate, fromlist=["GNSDEspatial"])
...
if _model_mod is None:
    ...
    print(f"\nFix: edit _CANDIDATE_MODULES near the top of this file to "
          f"match your actual model file's dotted import path.\n")
    raise ModuleNotFoundError(
        f"Could not find the model file under any of: {_CANDIDATE_MODULES}")

print(f"[test_correlated_noise] Using model module: {_MODEL_MODULE}")
...
def empirical_sample_correlation(samples_tensor, val_idx):
    """
    Cross-check: compute the EMPIRICAL cross-node correlation of MC
    sample residuals (around their own mean) on the validation window,
    independent of the model's learned Lambda. If the model's implied
    correlation structure is doing real work, this empirical estimate
    should broadly agree in sign/magnitude with what
    implied_correlation_matrix() reports -- not identical (finite-
    sample noise, and this mixes solver-path variation across the full
    validation window rather than a single instant), but directionally
    consistent. A large mismatch would suggest the loadings aren't
    actually shaping the trained model's realized sample paths the way
    intended.
    """
    val_start, val_end = val_idx[0], val_idx[-1]
