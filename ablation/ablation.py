...
# This script lives in a subfolder (e.g. ablation/run_ablations.py).
# Your project has a `src/` package root (not a top-level `model/`),
# based on paths used elsewhere in this codebase (e.g.
# src/eval/hawkes_rff_baseline.py). Put both the project root AND
# project_root/src on sys.path so `model...` resolves whether the
# package is `model.xxx` (root) or `src.model.xxx` -- and try both
# filenames used across this project's scripts
# (gnsde_new32_final_paper / gnsde_new32_final_paper_review), since
# different training scripts in this conversation have referenced
# both.
import sys
from pathlib import Path
