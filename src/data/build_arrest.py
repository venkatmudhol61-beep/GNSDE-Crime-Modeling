...
CONFIGS = {
    ...
    "nyc_precincts": {
        "path":        "data/raw/nyc_arrests.csv",
        "date_col":    "CMPLNT_FR_DT",
        "region_col":  "ADDR_PCT_CD",
        "arrest_col":  "ARREST_FLAG",
        "date_fmt":    "%m/%d/%Y",
        "year_min":    2006,
        "year_max":    2024,
        "century_fix": True,
...
def preprocess_arrest(dataset_name):
    ...
    # Clean arrest column — convert to numeric 0/1
    df[cfg["arrest_col"]] = pd.to_numeric(df[cfg["arrest_col"]], errors="coerce")
    df = df.dropna(subset=[cfg["arrest_col"]])

    # Aggregate — mean arrest rate per region per month
    arrest = (
        df.groupby(["region_id", "month"])[cfg["arrest_col"]]
          .mean()
          .reset_index(name="L")
    )

    arrest = arrest[["month", "region_id", "L"]]

    out_path = f"data/processed/{dataset_name}_arrest_timeseries.csv"
    arrest.to_csv(out_path, index=False)

    print(f"  Regions : {arrest['region_id'].nunique()}")
    print(f"  Months  : {arrest['month'].nunique()}")
    print(f"  Range   : {arrest['month'].min()} → {arrest['month'].max()}")
    print(f"  L min   : {arrest['L'].min():.4f}")
    print(f"  L max   : {arrest['L'].max():.4f}")
    print(f"  L mean  : {arrest['L'].mean():.4f}")
    print(f"  Rows    : {len(arrest):,}")
    print(f"  Saved  → {out_path}")

if __name__ == "__main__":
    for name in CONFIGS:
        preprocess_arrest(name)
